...
export default function locateWaterfox (
  ...
  const envPath = e?.WATERFOX_BINARY
...
const DEFAULT_INSTALL_STEPS: InstallGuidanceStep[] = [
  {
    summary: 'Install Waterfox from the official site',
...
export function getInstallGuidance (opts?: InstallGuidanceOptions): string {
  ...
  const lines = [
    "We couldn't find a Waterfox browser on this machine.",
    '',
    'To install one:',
    ''
  ]
  ...
  lines.push('')
  lines.push('Alternatively, set WATERFOX_BINARY=/path/to/waterfox and re-run.')

  return lines.join('\n')
}

export function locateWaterfoxOrExplain (
  options?: boolean | {allowFallback?: boolean}
): string {
  const allowFallback =
    typeof options === 'boolean' ? options : Boolean(options?.allowFallback)

  const found = locateWaterfox(allowFallback) || locateWaterfox(true)
...
/**
 * Cross-platform Waterfox version resolver.
 ...
 */
export function getWaterfoxVersion (
  bin: string,
